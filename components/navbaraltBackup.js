...
export default function NavbarAlt(props) {
  const menu = [
    ...
    {
      label: "Store",
      href: "https://store.fiskerinc.com/home",
      external: true
    }
  ];
            ...
            <>
              <div className="flex flex-wrap justify-between md:gap-10 lg:flex-nowrap">
                <div className="flex w-full items-center justify-between lg:w-auto">
                  <Link href="/" className="w-28 dark:hidden">
                    ...
                    <h1 class="centima mx-auto max-w-4xl text-3xl font-extrabold uppercase text-gray-800">
                      GLZE <span class="text-black text-xs">&copy;</span>
                    </h1>
                  </Link>
                ...
                <div className="flex items-center gap-3">
                  <div className="hidden w-full flex-col items-center lg:flex lg:w-auto lg:flex-row dm-sans-bold">
                    {menu.map((item, index) => (
                      <>
                        {item.children && item.children.length > 0 ? (
                          ...
                          <Link
                            href={item.href}
                            key={index + item.label}
                            className="rounded-full px-5 py-2 font-medium text-gray-600 outline-none ring-blue-100 hover:text-blue-500 focus-visible:text-blue-500 focus-visible:ring-2 dark:text-gray-400 dm-sans-bold"
              ...
              <Disclosure.Panel>
                <div className="order-2 -ml-5 mt-5 flex w-full flex-col items-start justify-start lg:hidden dm-sans-bold">
                  {menu.map((item, index) => (
                    <>
                      {item.children && item.children.length > 0 ? (
                        <DropdownMenu
                        ...
                        <Link
                          href={item.href}
                          key={index + item.label}
                          className="rounded-full px-5 py-2 text-sm font-medium text-gray-600 outline-none ring-blue-100 hover:text-blue-500 focus-visible:text-blue-500 focus-visible:ring-2 dark:text-gray-400 dm-sans-bold"
                          target={item.external ? "_blank" : ""}
                          rel={item.external ? "noopener" : ""}>
                          {item.label}
                        </Link>
          ...
          <Transition
            ...
            <Menu.Items
              className={cx(
                ...
                !mobile && "bg-white shadow-lg  dark:bg-gray-800 dm-sans-bold"
              )}>
                    ...
                    {({ active }) => (
                      <Link
                        ...
                        className={cx(
                          ...
                          active
                            ? "text-blue-500"
                            : "text-gray-700 hover:text-blue-500 focus:text-blue-500 dark:text-gray-300 dm-sans-bold"
                        )}>
